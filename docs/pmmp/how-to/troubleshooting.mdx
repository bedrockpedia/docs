...
description: Common issues and solutions for PocketMine-MP server problems.
---
...
**Problem**: Server crashes immediately, shows "Extension not loaded", or gives a version mismatch error.

**Solution**:
- PocketMine-MP requires strictly compatible **PHP binaries**
- Ensure you aren't using a PM4 PHP binary for a PM5 server (and vice-versa).
- Download the correct PHP compilation for your OS from the [PocketMine-MP PHP-Binaries repo](https://github.com/pmmp/PHP-Binaries/releases).
...
**Solution**:
- Delete `pocketmine.yml` and `server.properties`
...
2. **Check firewall**: Allow PocketMine-MP/PHP through Windows Firewall
...
**Solution**:
- **Allocate more RAM**: Edit the `memory-limit` inside `pocketmine.yml` to allow the PHP instance to use more RAM.
...
- Verify plugin compatibility with your PocketMine-MP API version
- Ensure the `.phar` file is in the `plugins/` folder (not a subfolder)
...
<Blockquote color="primary">
  Join the PocketMine-MP community for support:
