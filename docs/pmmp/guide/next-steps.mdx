---
title: Next Steps
description: Essential configuration and management steps for your new PocketMine-MP server.
---

# Next Steps

Congratulations on installing your PocketMine-MP server! Now that your server is running, here are the essential next steps to configure and manage it.
...
- **`max-players`**: The maximum number of players allowed to join (Default: `20`).
- **`gamemode`**: The default game mode (`SURVIVAL`, `CREATIVE`, `ADVENTURE`, `SPECTATOR`).
- **`white-list`**: Set to `on` to only allow specific players to join.
- **`xbox-auth`**: Set to `on` to require players to be authenticated with Xbox Live.
...
### Find Plugins
Explore [Poggit](https://poggit.pmmp.io/plugins) or the official PocketMine-MP community forums for compatible plugins.

### Install
1. **Stop** your server if it is running (type `stop` in console).
2. Download the plugin `.phar` file.
